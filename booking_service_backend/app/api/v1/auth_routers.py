...
async def login(body: UserLoginRequestSchema, session: AsyncSession = Depends(get_db)) -> UserAuthResponseSchema | None:
    ...
    if user:
        if bcrypt.checkpw(body.password.encode("utf-8"), user.hashed_password):
            ...
            return UserAuthResponseSchema(email=user.email, phone_number=user.phone_number, id=user.id, is_logined=True, full_name=user.full_name, jwt_token=jwt_token)
...
async def register(body: UserRegisterRequestSchema, session: AsyncSession = Depends(get_db)) -> UserAuthResponseSchema | None:
    new_user_dto = UsersCreateSchema(
    ...
    new_user: Users | None = await UsersRepo.create(session=session, inserting_data_dto=new_user_dto)

    if new_user:
        jwt_token = create_access_token(user_id=new_user.id)

        return UserAuthResponseSchema(
            email=new_user.email, phone_number=new_user.phone_number, id=new_user.id, is_logined=True, full_name=new_user.full_name, jwt_token=jwt_token
        )
