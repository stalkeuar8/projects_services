...
async def get_client_by_id(client_id: int, session: AsyncSession = Depends(get_db)) -> dict[str, Any]:
    ...
    if client:
        response_obj = ClientsResponseSchema(id=client.id, full_name=client.full_name, phone_number=client.phone_number)
...
async def create_client(body: ClientsCreateSchema, session: AsyncSession = Depends(get_db)) -> dict[str, Any]:
    new_client: Clients | None = await ClientsOrm.create(session=session, inserting_data_dto=body)

    if new_client:
        response_obj = ClientsResponseSchema(id=new_client.id, full_name=new_client.full_name, phone_number=new_client.phone_number)
...
async def delete_client(client_id: int, session: AsyncSession = Depends(get_db)) -> dict[str, Any]:
    ...
    if deleted_client:
        response_obj = ClientsResponseSchema(id=deleted_client.id, full_name=deleted_client.full_name, phone_number=deleted_client.phone_number)
