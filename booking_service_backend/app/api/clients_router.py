...
async def get_client_by_id(client_id: int, session: AsyncSession = Depends(get_db)) -> ClientsResponseSchema | None:
    client: Clients | None = await ClientsRepo.find_by_id(session=session, id_to_find=client_id)

    if client:
        return ClientsResponseSchema(id=client.id, full_name=client.full_name, phone_number=client.phone_number, email=client.email)
...
async def create_client(body: ClientsCreateSchema, session: AsyncSession = Depends(get_db)) -> ClientsResponseSchema | None:
    new_client: Clients | None = await ClientsRepo.create(session=session, inserting_data_dto=body)

    if new_client:
        return ClientsResponseSchema(
...
async def delete_client(client_id: int, session: AsyncSession = Depends(get_db)) -> ClientsResponseSchema | None:
