...
class HotelsOrm(BaseOrm):
    ...
    async def multi_create(
        incoming_data_list_dto: list[HotelsSchema], session: AsyncSession
    ):
        hotels = [Hotels(**hotel.model_dump()) for hotel in incoming_data_list_dto]

        session.add_all(hotels)

    @staticmethod
    async def find_by_id(id_to_find: int, session: AsyncSession):
        query = select(Hotels).filter_by(id=id_to_find)
    ...
    async def delete_by_id(id_to_delete: int, session: AsyncSession):
        query = delete(Hotels).where(Hotels.id == id_to_delete).returning(Hotels)
        result = await session.execute(query)
