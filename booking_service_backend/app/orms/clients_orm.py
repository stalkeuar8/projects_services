...
class ClientsOrm(BaseOrm):
    ...
    async def multi_create(
        incoming_data_list_dto: list[ClientsSchema], session: AsyncSession
    ):
        clients = [Clients(**client.model_dump()) for client in incoming_data_list_dto]
    ...
    async def multi_find_by_ids(id_to_find_list: list[int], session: AsyncSession):
        query = select(Clients).where(Clients.id.in_(id_to_find_list))
    ...
    async def find_by_name(name_element: str, session: AsyncSession):
        query = select(Clients).where(Clients.full_name.contains(name_element))
        results = await session.execute(query)
        clients = results.scalars().all()
        return clients
    ...
    async def find_by_phone_number(phone_number: str, session: AsyncSession):
        query = select(Clients).filter_by(phone_number=phone_number)
    ...
    async def delete_by_id(id_to_delete: int, session: AsyncSession):
        query = delete(Clients).where(Clients.id == id_to_delete).returning(Clients)
