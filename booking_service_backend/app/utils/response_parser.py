...
from app.schemas.rooms_schemas import RoomCategory, RoomsResponseSchema
...
def create_user_response(user_obj: Users) -> UsersResponseSchema:
    return UsersResponseSchema(id=user_obj.id, full_name=user_obj.full_name, phone_number=user_obj.phone_number, email=user_obj.email)


def create_hotel_response(hotel_obj: Hotels) -> HotelsResponseSchema:
    return HotelsResponseSchema(id=hotel_obj.id, name=hotel_obj.name, country=hotel_obj.country, city=hotel_obj.city, rating=hotel_obj.rating)
