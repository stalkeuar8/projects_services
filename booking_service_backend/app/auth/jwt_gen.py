...
async def get_current_user(jwt_token: str = Depends(oauth2_scheme), session: AsyncSession = Depends(get_db)) -> Users:
    ...
    if user_id:
        user: Users | None = await UsersRepo.find_by_id(id_to_find=int(user_id), session=session)
    ...
    else: raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="JWT Auth error, Could not validate credentials") 
